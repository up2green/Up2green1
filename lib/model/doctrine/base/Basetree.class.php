<?php

/**
 * Basetree
 * 
 * This class has been auto-generated by the Doctrine ORM Framework
 * 
 * @property integer $id
 * @property treeProgramme $Programme
 * @property Doctrine_Collection $userTree
 * 
 * @method integer             getId()        Returns the current record's "id" value
 * @method treeProgramme       getProgramme() Returns the current record's "Programme" value
 * @method Doctrine_Collection getUserTree()  Returns the current record's "userTree" collection
 * @method tree                setId()        Sets the current record's "id" value
 * @method tree                setProgramme() Sets the current record's "Programme" value
 * @method tree                setUserTree()  Sets the current record's "userTree" collection
 * 
 * @package    up2green
 * @subpackage model
 * @author     Clément Gautier
 * @version    SVN: $Id: Builder.php 7490 2010-03-29 19:53:27Z jwage $
 */
abstract class Basetree extends sfDoctrineRecord
{
    public function setTableDefinition()
    {
        $this->setTableName('tree');
        $this->hasColumn('id', 'integer', 4, array(
             'type' => 'integer',
             'primary' => true,
             'autoincrement' => true,
             'length' => 4,
             ));
    }

    public function setUp()
    {
        parent::setUp();
        $this->hasOne('treeProgramme as Programme', array(
             'local' => 'id',
             'foreign' => 'tree_id'));

        $this->hasMany('userTree', array(
             'local' => 'id',
             'foreign' => 'tree_id'));

        $timestampable0 = new Doctrine_Template_Timestampable(array(
             ));
        $this->actAs($timestampable0);
    }
}